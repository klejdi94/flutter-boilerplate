...
//Drawer con ListView.separated + GridView & BottomSheet
//Calendario con Drawer + ListView con separatori(per mesi) e GridView (per giorni) + es BottomSheet
class Esempio5 extends StatefulWidget {
  static const String routeName = '/selfImprovement/esempio5';

  @override
  _Esempio5State createState() => _Esempio5State();
}

class _Esempio5State extends State<Esempio5> {
    ...
    return Scaffold(
      ...
      body: Builder(
          builder: (context) => GridView.count(
              crossAxisCount: 2,
              childAspectRatio: 1,
              children: List.generate(month[_selectedMonthIndex].days.length,
                  (index) {
                return Card(
                  child: ListTile(
                    onTap: () {
                    ...
                    title: Text(
                        month[_selectedMonthIndex].days[index].nr.toString()),
                    subtitle: Text(
                        month[_selectedMonthIndex].days[index].task ??
                            "Nessuna Nota"),
                  ),
                );
              }))),
